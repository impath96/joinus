...
public class OauthService {
	...
	public MembersVo oauthLogin(String service, String code) {
		...
		MembersVo socialLoginMember = snsLogin.requestSocialMember(code);
		
		// 소셜 로그인 토큰으로 일단 체크 -> 있으면 바로 로그인 완료?
		// 3. 특정 조건으로 해당 회원이 가입자인지 체크 -> 이메일이 UK이니까 이메일로 체크?
		System.out.println("select 하기 전");
		MembersVo findMember = memberDao.selectMember(socialLoginMember.getMember_email());
